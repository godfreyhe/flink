...
class StreamTableEnvironmentImpl (
  ...
  override protected def translate(
    ...
    modifyOperations.asScala.map { operation =>
      val transformationList = planner.translate(Collections.singletonList(operation))
      Preconditions.checkArgument(transformationList.size == 1,
        s"transformation size is ${transformationList.size()}", null)
      transformationList.get(0)
    }.asJava
  }
