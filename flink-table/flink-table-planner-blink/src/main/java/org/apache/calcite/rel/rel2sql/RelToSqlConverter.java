...
public class RelToSqlConverter extends SqlImplementor implements ReflectiveVisitor {
    ...
    /**
     * Dispatches a call to the {@code visit(Xxx e)} method where {@code Xxx} most closely matches
    ...
    protected Result result(
        ...
        return super.result(node, clauses, neededAlias, neededType, aliases)
                .withAnon(isAnon())
                .withExpectedClauses(frame.ignoreClauses, frame.expectedClauses, frame.parent);
    }
    ...
    /**
     * A SqlShuttle to replace references to a column of a table alias with the expression from the
     * select item that is the source of that column. ANTI- and SEMI-joins generate an alias for
     * right hand side relation which is used in the ON condition. But that alias is never created,
     * so we have to inline references.
     */
    private static class AliasReplacementShuttle extends SqlShuttle {
        ...
        AliasReplacementShuttle(
                String tableAlias, RelDataType tableType, SqlNodeList replaceSource) {
    ...
    protected Result visitAntiOrSemiJoin(Join e) {
        ...
        if (leftResult.neededAlias != null) {
            SqlVisitor<SqlNode> visitor =
                    new AliasReplacementShuttle(
                            leftResult.neededAlias,
                            e.getLeft().getRowType(),
            ...
            if (existsSqlSelect.getWhere() != null) {
                sqlCondition =
                        SqlStdOperatorTable.AND.createCall(
                                POS, existsSqlSelect.getWhere(), sqlCondition);
            }
        ...
        if (sqlSelect.getWhere() != null) {
            sqlCondition =
                    SqlStdOperatorTable.AND.createCall(POS, sqlSelect.getWhere(), sqlCondition);
        }
        sqlSelect.setWhere(sqlCondition);
        final SqlNode resultNode =
                leftResult.neededAlias == null ? sqlSelect : as(sqlSelect, leftResult.neededAlias);
    ...
    public Result visit(Correlate e) {
        final Result leftResult =
                visitInput(e, 0).resetAlias(e.getCorrelVariable(), e.getRowType());
        ...
        final SqlNode rightLateralAs =
                SqlStdOperatorTable.AS.createCall(
                        POS, rightLateral, new SqlIdentifier(rightResult.neededAlias, POS));
    ...
    private Builder visitAggregate(Aggregate e, List<Integer> groupKeyList, Clause... clauses) {
        // "select a, b, sum(x) from ( ... ) group by a, b"
        final boolean ignoreClauses = e.getInput() instanceof Project;
        final Result x = visitInput(e, 0, isAnon(), ignoreClauses, ImmutableSet.copyOf(clauses));
        final Builder builder = x.builder(e);
        final List<SqlNode> selectList = new ArrayList<>();
     ...
     */
    protected void buildAggGroupList(
            Aggregate e, Builder builder, List<SqlNode> groupByList, List<SqlNode> selectList) {
     ...
     */
    protected Builder buildAggregate(
            Aggregate e, Builder builder, List<SqlNode> selectList, List<SqlNode> groupByList) {
    ...
    /**
     * Generates the GROUP BY items, for example {@code GROUP BY x, y}, {@code GROUP BY CUBE (x, y)}
     * or {@code GROUP BY ROLLUP (x, y)}.
     *
     * <p>Also populates the SELECT clause. If the GROUP BY list is simple, the SELECT will be
     * identical; if the GROUP BY list contains GROUPING SETS, CUBE or ROLLUP, the SELECT clause
     * will contain the distinct leaf expressions.
     */
    private List<SqlNode> generateGroupList(
            Builder builder,
        ...
        assert aggregate.getGroupSet().asList().equals(sortedGroupList)
                : "groupList "
                        + groupList
                        + " must be equal to groupSet "
                        + aggregate.getGroupSet()
                        + ", just possibly a different order";
        ...
        switch (aggregate.getGroupType()) {
            ...
            case OTHER:
                return ImmutableList.of(
                        SqlStdOperatorTable.GROUPING_SETS.createCall(
                                SqlParserPos.ZERO,
    ...
    private SqlNode groupItem(
            List<SqlNode> groupKeys, ImmutableBitSet groupSet, ImmutableBitSet wholeGroupSet) {
    ...
    public Result visit(Intersect e) {
        return setOpToSql(
                e.all ? SqlStdOperatorTable.INTERSECT_ALL : SqlStdOperatorTable.INTERSECT, e);
    }
    ...
    public Result visit(Calc e) {
        ...
        final ImmutableSet<Clause> expectedClauses =
                program.getCondition() != null ? ImmutableSet.of(Clause.WHERE) : ImmutableSet.of();
    ...
    public Result visit(Values e) {
        ...
        final boolean rename =
                stack.size() <= 1 || !(Iterables.get(stack, 1).r instanceof TableModify);
                ...
                for (int i = 0; i < e.getRowType().getFieldCount(); ++i) {
                    operandList.add(
                            castNullType(
                                    ...
                                    e.getRowType().getFieldList().get(i).getType()));
                }
                selects.add(ANONYMOUS_ROW.createCall(POS, operandList));
            } else {
                for (List<RexLiteral> tuple : e.getTuples()) {
    ...
    public Result visit(Sort e) {
        if (e.getInput() instanceof Aggregate) {
            ...
            if (hasTrickyRollup(e, aggregate)) {
                ...
                final Builder builder =
                        visitAggregate(
                                aggregate,
                                ImmutableList.copyOf(groupList),
                                Clause.GROUP_BY,
                                Clause.OFFSET,
                                Clause.FETCH);
        ...
        if (e.getInput() instanceof Project) {
            ...
            if (permutation != null && project.getInput() instanceof Aggregate) {
        ...
        final Result x = visitInput(e, 0, Clause.ORDER_BY, Clause.OFFSET, Clause.FETCH);
        final Builder builder = x.builder(e);
    ...
    /**
     * Adds OFFSET and FETCH to a builder, if applicable. The builder must have been created with
    ...
    private SqlNodeList exprList(final Context context, List<? extends RexNode> exprs) {
        return new SqlNodeList(Util.transform(exprs, e -> context.toSql(null, e)), POS);
    }
    ...
    private SqlNodeList identifierList(List<String> names) {
        return new SqlNodeList(Util.transform(names, name -> new SqlIdentifier(name, POS)), POS);
    }
    ...
    public Result visit(Match e) {
        ...
        for (Map.Entry<String, SortedSet<String>> entry : e.getSubsets().entrySet()) {
            ...
            subsetList.add(
                    SqlStdOperatorTable.EQUALS.createCall(POS, left, new SqlNodeList(rhl, POS)));
        }
    ...
    public Result visit(Uncollect e) {
        ...
        final List<SqlNode> operands =
                createAsFullOperands(e.getRowType(), unnestNode, x.neededAlias);
     ...
     * @param alias alias
     */
    public List<SqlNode> createAsFullOperands(
            RelDataType rowType, SqlNode leftOperand, String alias) {
    ...
    private static class Frame {
        ...
        Frame(
                ...
                boolean anon,
                boolean ignoreClauses,
                Iterable<? extends Clause> expectedClauses) {
