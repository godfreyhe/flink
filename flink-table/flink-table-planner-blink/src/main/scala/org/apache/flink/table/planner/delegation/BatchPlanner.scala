...
import org.apache.flink.table.planner.plan.nodes.process.{BatchMultipleInputCreationProcessor, DAGProcessContext}
...
class BatchPlanner(
  ...
  override private[flink] def translateToExecNodePlan(
    ...
    // breakup deadlock
    val execNodePlan1 = new DeadlockBreakupProcessor().process(execNodePlan, context)
    new BatchMultipleInputCreationProcessor().process(execNodePlan1, context)
  }
